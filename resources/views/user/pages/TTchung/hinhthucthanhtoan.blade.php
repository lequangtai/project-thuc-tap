      ...
      <div class="TTTT">
          ...
          <!-- <div class="box-right"> -->
            <div class="panel panel-primary" id="tin-HD">
              <div class="panel-heading">
                <h3 class="panel-title">HƯỚNG DẪN CÁC HÌNH THỨC THANH TOÁN TIỀN ĐIỆN</h3>
              </div>
              <div class="panel-body">
              @php
                  $news = App\Models\Cate::select()->where('parent_id',19)->paginate(5);
                   
                  @endphp
              @foreach($news as $item)
                  <li class="tin3">{!! $item->name!!}<img src="{!! asset('public/tt13_user/image/click_here.png')!!}" style="margin-right: 10px"><a href="{!! url('chitiethinhthucthanhtoan/'.$item['id']) !!}">Nhấp vào để xem chi tiết</a></li>
                @endforeach
              </div>
     </div>
    </div>
